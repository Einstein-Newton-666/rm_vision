...
: Node("armor_detector", options)
{
  ...
  recieve_sub_ = this->create_subscription<auto_aim_interfaces::msg::RecieveData>(
    "/recieve_pack", rclcpp::SensorDataQoS(),bind(&ArmorDetectorNode::rcvmsgCallback,this,std::placeholders::_1));
...
void  ArmorDetectorNode::rcvmsgCallback(const auto_aim_interfaces::msg::RecieveData::SharedPtr msg){
  if(msg->current_color==0)
  {
    maintaincolor=1;
    }
  else{maintaincolor=0;}
  if(sgn){
  detector_ = initDetector();
  sgn=false;}
...
{  
  ...
  if(!armors.empty()){
        RCLCPP_INFO(this->get_logger(), "detect armor!");
...
{
  RCLCPP_INFO(this->get_logger(), "Start initDetector!");
  ...
  Detector::LightParams l_params = {
    ...
    .max_angle = declare_parameter("light.max_angle", 60.0)};
  ...
  int detect_color = maintaincolor;
  std::cout<<"maintaincolor:"<<maintaincolor<<std::endl;
...
{ 
  ...
  std::cout<<"latancy:  "<<latency<<std::endl;
  ...
  //  RCLCPP_INFO(this->get_logger(), "enter debug!");
