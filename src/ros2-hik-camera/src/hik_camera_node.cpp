...
private:
  void declareParameters()
  {
    ...
    MV_CC_SetFloatValue(camera_handle_, "AcquisitionFrameRate", 165);

    //Gamma
    // 使能 = open
    MV_CC_SetBoolValue(camera_handle_,"GammaEnable", true);
    MV_CC_SetEnumValue(camera_handle_,"GammaSelector",1);//1:User 2:sRGB
    param_desc.description = "Gamma";
    MV_CC_GetFloatValue(camera_handle_, "Gamma", &f_value);
    param_desc.integer_range[0].from_value = 0.1;
    param_desc.integer_range[0].to_value = 4.0;
    double gamma = this->declare_parameter("gamma",0.6, param_desc);// default = 1; 0.0-4.0
    MV_CC_SetFloatValue(camera_handle_, "Gamma",gamma);
